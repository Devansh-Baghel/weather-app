const getWeatherFromCity = async (city) => {
  const response = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=48df9710e82d465da64214434232006&q=${city}&days=4`,
    { mode: "cors" }
  );
...
const IP = {
  ...
  getWeather: async () => {
    ...
    const response = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=48df9710e82d465da64214434232006&q=${ip}&days=4`,
      { mode: "cors" }
    );
    const data = await response.json();
    return data;
